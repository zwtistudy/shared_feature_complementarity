...
class Buffer():
    ...
    def __init__(self, config:dict, observation_space_shape:spaces.Box, action_space_shape:tuple, device:torch.device) -> None:
        """
        Arguments:
        ...
        self.actions = torch.zeros((self.n_workers, self.worker_steps, action_space_shape), dtype=torch.float32)
        self.dones = np.zeros((self.n_workers, self.worker_steps), dtype=bool)
        self.obs = [torch.zeros((self.n_workers, self.worker_steps) + obs_space, dtype=torch.float32) for obs_space in observation_space_shape]
        ...
        self.log_probs = torch.zeros((self.n_workers, self.worker_steps, action_space_shape), dtype=torch.float32)
        ...
        for key, value in samples.items():
            if not isinstance(value, list):
                ...
                for i in range(len(_value)):
                    # Split data into episodes or sequences
                    sequences, max_sequence_length = self._arange_sequences(_value[i], episode_done_indices)
                    ...
                    # Stack sequences (target shape: (Sequence, Step, Data ...) & apply data to the samples dict
                    value.append(torch.stack(sequences, axis=0))
        ...
        for key, value in samples.items():
            if not isinstance(value, list):
                ...
                if not key == "hxs" and not key == "cxs":
                    ...
                    value = []
                    for i in range(len(_value)):
                        _value[i] = _value[i].reshape(_value[i].shape[0] * _value[i].shape[1], *_value[i].shape[2:])
    ...
    def _pad_sequence(self, sequence:np.ndarray, target_length:int) -> np.ndarray:
        ...
        if isinstance(sequence, torch.Tensor):
            ...
            for i in sequence:
                if len(i.shape) > 1:
                    # Case: pad multi-dimensional array (e.g. visual observation)
                    padding = torch.zeros(((delta_length,) + i.shape[1:]), dtype=i.dtype)
                else:
                    padding = torch.zeros(delta_length, dtype=i.dtype)
                # Concatenate the zeros to the sequence
                new_sequence.append(torch.cat((i, padding), axis=0))
            return new_sequence
            ...
            for key, value in self.samples_flat.items():
                if not isinstance(value, list):
                    ...
                    if key == "hxs" or key == "cxs":
                        # Select recurrent cell states of sequence starts
                        # mini_batch[key] = value[sequence_indices[start:end]].to(self.device)
                        mini_batch[key] = [i[sequence_indices[start:end]].to(self.device) for i in value]
                    elif key == "log_probs" or "advantages" in key or key == "values":
                        # Select unpadded data
                        # mini_batch[key] = value[mini_batch_unpadded_indices].to(self.device)
                        mini_batch[key] = [i[mini_batch_unpadded_indices].to(self.device) for i in value]
                    else:
                        # Select padded data
                        # mini_batch[key] = value[mini_batch_padded_indices].to(self.device)
                        mini_batch[key] = [i[mini_batch_padded_indices].to(self.device) for i in value]
            start = end
            yield mini_batch
